...
author: "Nils Eling (Adopted from Daniel Schulz / Lasse Meyer)"
date: "`r Sys.Date()`"
output: 
  html_document:
    toc: true
...
Another spill over compensation approach has been published by [Guazzini et
al](https://academic.oup.com/bioinformatics/article/40/6/btae337/7689679) which
also uses the experimentally generated spill over matrix but works on the
assumption that the distribution of spill over per channel carries over to
neighboring channels. We have not gathered experience with this approach yet
besides that it is computationally more intense. A detailed description can be
obtained from bioconductor
[here](https://www.bioconductor.org/packages/release/bioc/html/spillR.html). For
simplicty, here we will focus on our well established approach.

When downloading the data you retrieved an MCD file which contains individually 
measured metal spots.

**NOTE**: the XTi generation of IMC instruments does not generate txt files any more. We have therefore recently added a new function to `imcRtools` package to generate the spill over matrix directly from tiff files. For this purpose the mcd files containing the spill over data needs to be processed with `steinbock`. We will highlight this converging workflow below.
...
sce <- readSCEfromTXT("data/compensation/txt/") 
assay(sce, "exprs") <- asinh(counts(sce)/5)

# Create SingleCellExperiment from images that were generated using the steinbock framework
sceTIFF <- readSCEfromTIFF(x = "data/compensation/img",
                           image_df_path = "data/compensation/images.csv",
                           panel_df_path = "data/compensation/panel.csv")
assay(sceTIFF, "exprs") <- asinh(counts(sceTIFF)/5)
```
...
# Log10 median pixel counts per spot and channel. We can also see that the data from either images or txt files is identical.
plotSpotHeatmap(sce)
plotSpotHeatmap(sceTIFF)
...
```{r debarcoding, warning = FALSE, message=FALSE}
library(CATALYST)
...
# tiff based
bc_key <- as.numeric(unique(sceTIFF$sample_mass))
bc_key <- bc_key[order(bc_key)]

sceTIFF <- assignPrelim(sceTIFF, bc_key = bc_key)
sceTIFF <- estCutoffs(sceTIFF)
sceTIFF <- applyCutoffs(sceTIFF)
```
...
sceTIFF <- filterPixels(sceTIFF, minevents = 40, correct_pixels = TRUE)
```
...
sceTIFF <- computeSpillmat(sceTIFF)
colnames(metadata(sceTIFF)$spillover_matrix)[1] <- "Ar80Di"
isotope_list <- CATALYST::isotope_list
isotope_list$Ar <- 80

plotSpillmat(sce, isotope_list = isotope_list)
plotSpillmat(sceTIFF, isotope_list = isotope_list)
# as we can see the two spillover matrices are identical and we will now proceed with only one matrix.
...
```{r}
library(tiff)
panel <- read.csv("data/cell_mode/panel.csv")
...
**NOTE** The compensation of larger numbers of images can be time consuming in R. We are working on a python based image spillover correction which would likely precede all single cell processing and thus also make the spillover correction from above obsolete.
...
This is more time consuming than merging sequentially based on how batches appear in the dataset (default). 
...
```{r visualizing-batch-correction-fastMNN-1, message=FALSE, warning=FALSE, fig.height=3, fig.width=7}
library(cowplot)
library(dittoSeq)
library(viridis)
...
plot_list <- multi_dittoDimPlot(spe, var = markers, reduction.use = "UMAP_mnnCorrected", 
                   assay = "exprs", size = 0.2, list.out = TRUE) 
plot_list <- lapply(plot_list, function(x) x + scale_color_viridis())
plot_list[["patients"]]  <- p2
plot_grid(plotlist = plot_list) 
```

We observe that immune cells across patients are merged after batch correction 
using `fastMNN` but they were also mostly merged before batch correction. The tumor cells of different patients on the other hand are more clustered after batch correction despite still the tumor cells from patient 3 cluster separately. 

# Cell phenotyping 

A common step during single-cell data analysis is the annotation of cells based
on their phenotype. Defining cell phenotypes is often subjective and relies on
previous biological knowledge. The [Orchestrating Single Cell Analysis with
Bioconductor](https://bioconductor.org/books/3.14/OSCA.basic/cell-type-annotation.html)
book presents a number of approaches to phenotype cells detected by single-cell
RNA sequencing based on reference datasets or prior knowledge of geneset
importance.
...
approach to group cells based on their similarity in marker expression.
...
data, compared to cytometry or single-cell RNAseq data were cells are physically separeted before measurements. At the end of this session we will train and apply a random
...
present a popular clustering approach that groups cells based on their
similarity in marker expression. A number of approaches have been developed to
cluster data derived from single-cell RNA sequencing technologies or CyTOF. For
demonstration purposes, we will only highlight the `Phenograph` clustering
approach. For a full overview on different clustering approaches, please refer
to the
[Phenotyping](https://bodenmillergroup.github.io/IMCDataAnalysis/cell-phenotyping.html#cell-phenotyping)
...
This clustering strategy is widely used in many IMC studies.
...
dataset and is used in the [IMMUcan project](https://immucan.eu/).
Recent studies have also shown that generally tree-based classification approaches outperform many other cell phenotyping approaches:  
[Eling et al., 2025](https://www.cell.com/cell-reports-methods/fulltext/S2667-2375(25)00206-1)
[Bussi et al., 2025](https://www.biorxiv.org/content/10.1101/2025.05.05.652215v1)
[spCellEval](https://huggingface.co/spaces/Arozhada/spcelleval)
...
[zenodo.org/record/7432486](https://zenodo.org/record/7432486) and will be downloaded below. Per image, the `cytomapperShiny` function allows the export of gated cells in form of a `SingleCellExperiment` or
...
# Double assignment within the tumor - there are 276 double labelled cells
...
# Remove remaining doublets (e.g. cells labelled as Stroma and CD8...)
cur_tab[cur_labels == "doublets",]
...
**Note** this functions takes about 7 min on a recent processor.
...
## Visualization of cell phenotypes
...
and images in form of barplots.
...
For easier interactive visualization of cell phenotyping results, we can again use 
the `cytoviewer` package. 
...
1. Compare the overlap between phenograph clustering and classification. Plot a heatmap comparing overlap between clusters as frequencies. Hint: `pheatmap`offers easy heatmap plotting functionality.

2. Compare the mean expression of markers per classified cell type across the 4 different tumor indications. as a heatpmap. Hint: `aggregateAcrossCells` and `dittoHeatmap` will help do this job.

3. Identify images with HLADR positive (exprs > 3) tumor cells. What's the percentage of tumor cells per image that are HLADR positive? Select the image with the highest percentage of HLADR positive tumor cells. Plot the HLADR, CD163 and Ecad expression (using plotPixels of that image) and discuss whether there truly HLADR positive tumor cells. Hint: use the exprs assay to generate a new `colData` entry and generate a new spe with only tumor cells. the `prop.table` function will help you to investigate the proportions of HLADR pos tumor cells and identify the patient image of interest. After that, subset masks, images and spe and use the `plotPixels` function to investigate the expression.
