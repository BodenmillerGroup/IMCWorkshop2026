...
cellular neighborhoods. For this we can use the `plotSpatial` function:
...
We set the seed within the `SerialParam` function for reproducibility. 
...
We can also use the `plotInteractions` function is used to visualize interaction networks from an
edge list, typically the output of the `testInteractions` function. 
...
edge_color <- setNames(c("red", "blue"),c("attraction", "repulsion"))
...
We will annotate the images with BnT cell patches as a surrogate for tertiary lymphoid (TLS)-like structures. 
...
Check for B cells interactions

```{r}
# TLS_like 
out$TLS <- spe$TLS_like[match(out$group_by, spe$sample_id)] 

out %>% filter(pair_id == "BnTcell__Bcell") %>%
ggplot()+
    facet_wrap(~pair_id)+
    geom_boxplot(aes(TLS, zscore, fill = TLS), outliers = FALSE)+
...
plotSpatial(spe[,spe$sample_id %in% patch_images], 
...
library(ggridges)
ggplot(as.data.frame(colData(spe)) %>% filter(TLS_like == "TLS_like")) + 
