...
around a given cell type ("point process") or utilize interaction graphs 
...
output `DataFrame` from our `testInteraction` results. 
...
from BnT cells to themselves (homotypic interaction) but also B cells/tumor cells (heterotypic interaction).

```{r TLS-diffinteraction, warning = FALSE}
out_filtered <- out %>% filter(pair_id %in% c("BnTcell__BnTcell", "BnTcell__Bcell", "BnTcell__Tumor"))
