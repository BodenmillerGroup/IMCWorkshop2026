...
cell-to-cell interactions. Spatial methods can also be broadly divided into more 
unsupervised and supervised approaches, which we will discuss below.   
...
# Unsupervised spatial analysis {#unsupervised}
...
2.  `method = "conditional"`: Formerly named "histocat". 
    ...
    "How many neighbors of type B has a cell of type A on average, 
...
# Supervised spatial analysis {#supervised}
...
We visualize the minimum distances to TLS-like patches in a cell type specific manner.
...
A) Visualize the cell type composition of CN3 (based on celltypes) across all images. 
Hint: You can use the `plotSpatial` function and the `colData` entry "cn_celltypes". 
...
Hint: You can use `table` and `pheatmap` functions. 

**TASK 3 - Cell-to-cell interaction analysis**: 
Perform cell-to-cell interaction analysis with a different counting approach 
(e.g. `histocat`) and compare your findings. Does the interpretation change? 

**TASK 4 - Patch detection and differential interaction testing**:
A) Visualize and compare the zscore of heterotypic interactions of BnT cells to 
B cells as well as BnT cells tumor cells between images with and without 
TLS-like structures. What do you observe? Hint: You can use the `ggplot` function. 
...
approaches below for further reading:
